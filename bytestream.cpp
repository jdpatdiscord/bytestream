...
inline void Bitstream::ResizeNeeded(const size_t SizeNeeded)
{
...
Bitstream::Bitstream() : Offset(0), CurrentAllocated(1)
{
...
template <typename T> const size_t Bitstream::EncodedSize(T Value) const
{
    size_t ValueSize = 0;
...
template <typename T> T Bitstream::ReadRaw()
{
...
template <typename T> T Bitstream::ReadEnc()
{
...
template <typename T> T Bitstream::ArbitraryReadRaw(const size_t ReadOffset, size_t* NextOffset = nullptr)
{
    static_assert(std::is_scalar_v<T>, "not a trivial (scalar) type");
...
template <typename T> T Bitstream::ArbitraryReadEnc(size_t ReadOffset, size_t* NextOffset = nullptr)
{
    static_assert(std::is_scalar_v<T>, "not a trivial (scalar) type");
...
template <typename T> void Bitstream::WriteRaw(T Value)
{
...
template <typename T> void Bitstream::WriteEnc(T Value)
{
    static_assert(std::is_arithmetic_v<T>, "not an operable type (arithmetic not supported)");
